...
export default function ExpertsSpeakersComp() {
  ...
  const fetchExperts = async (page = 0) => {
    setLoading(true);
    try {
      const response = await fetch(`https://graduation-page.onrender.com/experts?page=${page}&size=10`);
      if (!response.ok) {
      ...
      <div className="expertContainer">
        {experts.map((expert, id) => (
          <div className="expertCard" key={id}>
            <h2>{expert.name}</h2>
            <a href={expert.linkedin} target="_blank" rel="noopener noreferrer" className="expertLinkedInLink">
              LinkedIn Profile
            </a>
            <p>{expert.field}</p>
